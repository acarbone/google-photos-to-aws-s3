...
1. **Identify the gap**: What specifically went wrong? Which step in the `SKILL.md` procedure was missing, ambiguous, or wrong?
2. **Edit the `SKILL.md`**: Update the procedure to close the gap. Be precise — vague instructions produce vague output.
...
2. **Create the skill folder**: `mkdir .cursor/skills/<skill-name>/`
3. **Write `SKILL.md`**: Follow this structure:
   ```markdown
   ---
   name: <skill-name>
   ...
   ```
4. **Register the skill**: Add it to the skill index in `SKILLS.md`.
...
2. **If merging**: Incorporate any unique steps into the surviving skill's `SKILL.md`.
3. **Delete the folder**: Remove `.cursor/skills/<skill-name>/`.
